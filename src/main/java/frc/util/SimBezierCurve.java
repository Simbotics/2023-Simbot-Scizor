...
public class SimBezierCurve {
  ...
  public SimBezierCurve(SimPoint... p) {
    if (p.length < 2)
      throw new IllegalArgumentException(
          "Minimun number of interpolation points is 2, you have: " + p.length);
  ...
  public static double getPoint(double t, double... p) {
    double oldT = t;
    if (p.length == 2) return p[0] + (p[1] - p[0]) * t;

    if (0 > t || t > 1) t = -t;
    if (p.length < 2)
      throw new IllegalArgumentException(
          "Minimum number of interpolation points is 2, you have: " + p.length);

    final double[] d = new double[p.length];
    for (int i = 0; i < d.length; i++) d[i] = p[i];

    for (int i = d.length; i > 0; i--)
      for (int j = 1; j < i; j++) d[j - 1] = getPoint(t, d[j - 1], d[j]);
